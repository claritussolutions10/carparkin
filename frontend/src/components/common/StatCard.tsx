...
export default function StatCard({ label, value, sub, accent, icon, iconClassName }: StatCardProps) {
  return (
    <div className={`rounded-xl border p-5 ${accent ? 'bg-green border-green' : 'bg-white border-line'}`}>
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          <p className={`text-sm ${accent ? 'text-white/70' : 'text-ink/50'}`}>{label}</p>
          <p className={`font-display text-2xl font-semibold mt-1 ${accent ? 'text-white' : 'text-navy'}`}>{value}</p>
